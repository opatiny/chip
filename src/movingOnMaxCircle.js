...
board.on("ready", async function () {
    ...
    while (true) {
        for (let i=0; i<degrees.length; i++) {
            ...
            let angle1 = Math.round( 5 + oneServoDegree * degrees[i] );
            let angle2 = Math.round( 5 + oneServoDegree * degrees[(i + degrees.length/3) % degrees.length] );
            let angle3 = Math.round( 5 + oneServoDegree * degrees[(i + degrees.length/3*2) % degrees.length] );
